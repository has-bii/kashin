...
import { categoryBreakdownQuery, recentQuery, summaryQuery, trendsQuery } from "./dto"
...
export const dashboardController = new Elysia({ prefix: "/dashboard" })
  ...
  .get("/trends", async ({ user, query }) => DashboardService.trends(user.id, query.months ?? 6), {
    auth: true,
    query: trendsQuery,
  })
  .get("/recent", async ({ user, query }) => DashboardService.recent(user.id, query.limit ?? 5), {
    auth: true,
    query: recentQuery,
  })
